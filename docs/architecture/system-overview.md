...
```mermaid
flowchart LR
    A[Fuente externa\nRSS / sitio / API] --> B[Radar\nDiscoveryEngine]
    B -->|NewsFound| C[Extractor]
    C --> D[Writer\nreescritura con IA]
    D --> E[SEO]
    E --> F[Images]
    F --> G[WordPress\nBORRADOR]
...
**Estado actual (Sprint 2):** solo el primer tramo existe — Radar/
`DiscoveryEngine` (ver
[docs/architecture/discovery-engine.md](discovery-engine.md)). Todo lo
demás en el diagrama es diseño, no código.

## Agentes y responsabilidades

| Agente | Responsabilidad | Port principal | Estado |
|---|---|---|---|
| `radar` | Detectar noticias nuevas | `ContentSource`, `Repository`, `DiscoveryEngine` | 🟡 Motor listo, agente no |
